...
script_dir="/var/opt/mssql/northwind"
sqlfile="SqlServer_data.sql"

docker pull mcr.microsoft.com/mssql/server:2017-latest
docker run -e "ACCEPT_EULA=Y" -e "MSSQL_SA_PASSWORD=${password}" --name $container_name -p 1401:1433 -v sql1data:/var/opt/mssql -d mcr.microsoft.com/mssql/server:2017-latest
...
docker exec -it $container_name /opt/mssql-tools/bin/sqlcmd -S localhost -U SA -P $password -i "${script_dir}/${sqlfile}"